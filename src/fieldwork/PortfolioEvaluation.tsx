...
export default function PortfolioEvaluation({ activity, onEvaluated }: {
  ...
  const canRefresh = done && activity.evaluationVersion !== 3;
  const evaluate = async () => {
    if (pending.current || (done && !canRefresh)) return;
  ...
  return <section className="portfolio-evaluation" aria-label="프로젝트 AI 피드백" aria-busy={busy}>
    ...
    <p>{canRefresh ? "이전 방식의 요약이 저장되어 있어요. 아래 버튼을 누르면 잘한 점·보완점·다음 행동을 담은 피드백으로 갱신해요." : done ? "네가 만든 결과물에서 다음 아이디어를 찾아봤어." : "작성한 설명과 설계도 속 글·연결을 읽고, 잘 드러난 점과 다음에 다듬을 점을 알려줄게."}</p>
    {done && !!activity.observations?.length && <ul>{activity.observations.map((item, index) => <li key={index}>{item}</li>)}</ul>}
    {(!done || canRefresh) && <button className="button primary" disabled={busy} onClick={() => void evaluate()}>{busy ? "크랩이 결과물을 읽는 중…" : error ? "AI 피드백 다시 요청" : canRefresh ? "개선된 코치 피드백 받기" : "AI 피드백 받기"}</button>}
