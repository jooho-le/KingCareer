...
현재 **잇다**를 가칭으로 프론트엔드 체험판을 구현했습니다. 아래에는 서비스 기획과 구현 범위를 함께 정리했습니다. 서비스명은 추후 확정합니다.

## 웹앱 실행

Node.js 22.12 이상 환경에서 실행합니다.
...
브라우저 테스트는 로컬 Chrome을 사용합니다. 개발 서버가 실행 중이지 않으면 테스트가 자동으로 시작합니다.

### 구현한 화면

| 영역 | 연결한 기능 |
| --- | --- |
| 홈 | 경험 현황, 진로지도 진입, 추천 직무체험·프로젝트 |
| 온보딩·프로필 시작 | 서비스 소개, 학생·교사 선택, 관심분야·학교·학년·지역 입력 |
| 진로 진단·지도 | 6개 영역 자가 진단, 직업별 경험 지도, 활동 전후 변화·히스토리 |
| 직무체험 | 5개 직업, 직업별 3개 상황, 선택별 응답, 자유 답변, 회고·관심 변화 기록 |
| 미니 프로젝트 | 직업별 3단계 작성, 힌트, 초안 자동 저장, 제출·포트폴리오 반영 |
| 직업·지역 탐색 | 분야·검색 필터, 저장한 직업, 관련 전공·직업, 전북 지역별 체험 연결 |
| 다음 경험 추천 | 관심분야와 활동 수 기반 제안 및 추천 이유 |
| 포트폴리오 | 활동 필터, 선택·결과물·회고 상세, 텍스트 다운로드·요약 복사 |
| 교사·멘토 | 가상 학생의 경험 현황, 경험 공백 필터, 활동 배정 기록, 변화 리포트 |
| 마이페이지 | 프로필·관심분야 수정, 저장한 직업, 안내 알림 설정, 로컬 기록 삭제 |

### 체험판 범위

React, TypeScript, Vite와 Motion으로 구성한 프론트엔드입니다. 주황·보라·파랑 단색을 중심으로 카드 반응, 페이지 전환, 진행 애니메이션을 적용했습니다. 모바일 화면과 시스템의 움직임 줄이기 설정을 지원합니다.

프로필·활동·프로젝트 초안은 이 브라우저의 `localStorage`에 저장합니다. 실제 회원가입·로그인, 계정 복구·기기 간 동기화, 생성형 AI 호출, 실시간 기업 정보, 학생 관리 서버는 아직 연결하지 않았습니다. 교사 화면에는 가상 학생만 표시합니다.

직무체험은 준비된 시나리오와 선택별 응답을 사용하며, 프로젝트 피드백과 추천도 규칙 기반입니다. 경험 지표는 자가 응답과 활동 완료에 따라 표시하는 체험판 지표로, 검증된 심리검사·능력 평가가 아닙니다. 현직자 교류는 가상 활동으로 증가하지 않습니다.

화면별 경로와 서버 연결 예정 사항은 [프론트엔드 IA](docs/front-ia.md)에 정리했습니다.
